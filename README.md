...
Revenyu MCP allows your Frappe Framework app to function as a [Streamable HTTP MCP
...
> [!IMPORTANT]
>
> Revenyu MCP is in a highly experimental state, there will be bugs, breaking
...
Revenyu MCP is yet in its infancy, as of now it **only supports** Tools.
...
Revenyu MCP is fairly straightforward to use. Most of the MCP specific heavy
...
> allows Revenyu MCP to extract the `inputSchema` for the tool without any additional
> configuration.
...
> If this is not done your tools will not be loaded as Revenyu MCP does not know where
