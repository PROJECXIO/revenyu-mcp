...
from revenyu_mcp.cli import utils


@click.group(invoke_without_command=True)
@click.pass_context
def run(ctx):
    """Revenyu MCP CLI tool"""
...
def check(app: str | None = None, verbose: bool = False):
    """Check if Revenyu MCP is being used correctly"""
    try:
    ...
    if not app:
        apps = utils.get_apps_using_revenyu_mcp()
        if apps:
            click.echo(
                f'Found [{click.style(", ".join(apps), bold=True)}] that may be using {click.style("revenyu_mcp", "blue")}'
            )
            print()
    else:
        apps = [app]
    ...
    if not apps:
        click.secho('No apps found using revenyu_mcp', fg='yellow')
