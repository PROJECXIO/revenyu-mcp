...
from revenyu_mcp.server import MCP
...
def get_apps_using_revenyu_mcp() -> list[str]:
    """Find all apps installed on the Frappe bench that depend on revenyu-mcp."""
    from pathlib import Path
    ...
    for app_path in apps_path.iterdir():
        ...
        if any('revenyu-mcp' in dep for dep in dependencies):
