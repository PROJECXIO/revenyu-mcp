...
import revenyu_mcp.server.handlers as handlers
import revenyu_mcp.server.tools as tools
from revenyu_mcp.server import types
...
class MCP:
    ...
    def add_tool(self, tool: tools.Tool):
        ...
        be a dictionary conforming to the `revenyu_mcp.Tool` `TypedDict` structure.
    ...
    def _handle_request(
        ...
        match method:
            case 'initialize':
                result = handlers.handle_initialize(params, self._name or 'revenyu-mcp')
            case 'ping':
